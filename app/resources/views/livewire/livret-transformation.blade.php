...
$wire.UnValideElementsDuParcours( {{$user->id}} , selected_compagnonnages , selected_taches , selected_objectifs ,selected_sous_objectifs );">Annuler la validation des éléments cochés</button>
        ...
        <div id='livret' class='div-table-contrat-compagnonnage table'>
            ...
            <div x-data='{ show_fonction_{{$fonction->id}} : false}'>
                <h3 x-on:click="show_fonction_{{$fonction->id}}  = ! show_fonction_{{$fonction->id}} ">Fonction {{$fonction->fonction_liblong }} </h3>
                <div  x-show="show_fonction_{{$fonction->id}} ">
                    <table class='table'>
                    ...
                    <h4>Compagnonnages liés à la fonction {{ $fonction->fonction_liblong }} </h4>
                        @foreach($fonction->compagnonages()->get() as $compagnonage)
                            <div x-data='{ show_compagnonage_{{$compagnonage->id}} : false }'>
                                <div x-on:click="show_compagnonage_{{$compagnonage->id}} = !show_compagnonage_{{$compagnonage->id}};
                                    console.log('toto');">
                                    <h5>{{$compagnonage->comp_libcourt}}</h5>
                                </div>
                                <div x-show="show_compagnonage_{{$compagnonage->id}}">
