...
class GenerateStatistics extends Command
{
    ...
    public function handle()
    {
        ...
        $date_max = date('Y-m-30', mktime(0));
        $date_min = date('Y-m-1', mktime(0));
        $this->info("Statistiques du " . $date_min . " jusqu'au " . $date_max);
        // $date_max = date('Y-m-d', mktime(0,0,0,8,31,2022));
        // $date_min = date('Y-m-d', mktime(0,0,0,8,1,2022));
        // $this->info($date_max);
        
        $unite = Unite::find(intval($this->argument('unite')));
        ...
        foreach ($users as $user)
        {
            ...
            $date_embarq = new DateTime($user->date_embarq);
            $date_debarq = new DateTime($user->date_debarq);
            $nb_jour_gtr = $date_debarq->diff($date_embarq)->format("%a");
            // $this->info($date_embarq->format("D, d M Y"));
            ...
            if (!is_null($fonction_a_quai)){
                if (!is_null($fonction_a_quai->pivot->date_lache)){
                    $date_validation = new DateTime($fonction_a_quai->pivot->date_lache);
                    $nb_jour_quai= $date_validation->diff($date_embarq)->format("%a");
            ...
            if (!is_null($fonction_a_mer)){
                if (!is_null($fonction_a_mer->pivot->date_lache)){
                    $date_validation = new DateTime($fonction_a_mer->pivot->date_lache);
                    $nb_jour_mer= $date_validation->diff($date_embarq)->format("%a");
            ...
            if (!is_null($fonctions_metier)){
                ...
                if (!is_null($latest_validation_date)){
                    $date_validation = new DateTime($latest_validation_date->pivot->date_lache);
                    $nb_jour_metier= $date_validation->diff($date_embarq)->format("%a");
